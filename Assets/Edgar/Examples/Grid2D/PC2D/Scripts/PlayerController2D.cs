...
namespace Edgar.Unity.Examples.PC2D
{
    ...
    public class PlayerController2D : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (InputHelper.GetKeyDown(Input.JUMP))
            {
                _motor.Jump();
                _motor.DisableRestrictedArea();
            }

            _motor.jumpingHeld = InputHelper.GetKey(Input.JUMP);

            // XY freedom movement
            if (_motor.motorState == PlatformerMotor2D.MotorState.FreedomState)
            {
                _motor.normalizedXMovement = InputHelper.GetHorizontalAxis();
                _motor.normalizedYMovement = InputHelper.GetVerticalAxis();

                return; // do nothing more
            }

            // X axis movement
            if (Mathf.Abs(InputHelper.GetHorizontalAxis()) > Globals.INPUT_THRESHOLD)
            {
                _motor.normalizedXMovement = InputHelper.GetHorizontalAxis();
            }
            else
            ...
            if (InputHelper.GetKeyDown(Input.DASH))
            {
                _motor.Dash();
            }
        }
